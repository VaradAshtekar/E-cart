...
def categorywise(request):
    ...
    sub_cat = Product.objects.values('category','sub_category','id')
    sub_cats = {items['sub_category'] for items in sub_cat}

    for i in sub_cats:
        sub_prod = Product.objects.filter(sub_category=i)

        n = len(sub_prod)
        nslides = n // 4 + ceil((n / 4) - (n // 4))
        # print(sub_prod)
        allProds.append([sub_prod, range(1, nslides), nslides])
    params = {'allProds': allProds}
    # print(sub_cats)
    # print(sub_cat)
...
def search(request):
    ...
    # print(allProds)
    # nslides = 0
    # params = {'allProds' : [ Product.objects.filter(product_name=item['product_name']), range(1, nslides), nslides] for item in pro_names if query.lower() == item['product_name'].lower()}
    
    for items in pro_names:
        if query.lower() == items['product_name'].lower():
            allProds = []
